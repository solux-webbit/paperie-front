...
function ResultLaw({ results }) {
  ...
  const handleCellClick = (lawTitle) => {
    /* console.log('선택된 제목: ', lawTitle);
    axios.get(``)
    .then(response => {
          ...
          {results && results.length > 0 ? (
            results.map((result, index) => (
              <tr key={index} className="result_name">
                <td onClick={() => handleCellClick(result.lawTitle)}>{result.lawTitle}</td>
              </tr>
            ))
          ) : (
            <tr>
