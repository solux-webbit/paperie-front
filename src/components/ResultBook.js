...
function ResultBook({ results }) {
  ...
  const handleCellClick = (bookTitle) => {
    console.log('선택된 제목: ', bookTitle);
    axios.get(`http://127.0.0.1:8000/apa/books?selected_title=${bookTitle}`)
    .then(response => {
          ...
          {results && results.length > 0 ? (
            results.map((result, index) => (
              <tr key={index} className="result_name">
                <td onClick={() => handleCellClick(result.bookTitle)}>{result.bookTitle}</td>
              </tr>
            ))
          ) : (
            <tr>
